...
> 入口：[../SKILL.md](../SKILL.md) ｜ 配套 SOP：[sop-daily.md](./sop-daily.md) · [sop-fixup.md](./sop-fixup.md)
...
## 1. POST /api/baseline/input — 上传单条原子输入（最常用）
...
## 2. POST /api/baseline/input/batch — 批量上传
...
## 3. POST /api/baseline/override — 人工修正
...
## 4. POST /api/baseline/reset/:date — 完全重写
...
> **注意**：`reset` 不会清理 `future_watchlist`。未来观察项是全局数据，按 `expected_time` 在多天快照中展示。如需清理某条未来事件，请单独调用 `POST /api/baseline/future/:id/status` 标 `expired`。
