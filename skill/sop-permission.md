...
推理出**下一交易日**的 `状态 / 最大仓位 / 允许模式 / 禁止动作 / 触发停手条件`，写入一张卡。

> **核心思想**：错误不是记下来，而是**自动扣权限**。把行为模式映射成下一交易日的禁止动作。

> **写入时机与日期约定**：
> - 写卡的 `date` 字段 = **下一个交易日**（不是 today）。例：周一 16:00 写卡 → date=周二；周五 16:00 写卡 → date=下周一。
> - 前端徽章会按"盘前显示今日 / 盘后显示明日"自动切换：
>   - 周二 14:00（盘前）→ 用户看到「📅 今日权限」（指向同一张卡）
>   - 周二 15:30（盘后）→ 用户看到「🛑 明日权限」（指向下一张待写的卡）
> - Agent 的写入节奏建议：每个交易日收盘后 ~16:00 跑一次，写"下一交易日"的卡。
...
> 第二天用户在不同时段看到的徽章会自动切换语义：
> - **盘前 09:00**：`📅 今日权限 保护 仓50% 禁:补仓/倒T`（同一张卡，提醒今日要执行）
> - **盘后 15:30**：`🛑 明日权限 ... 未生成`（提醒 agent 该写下一张了）
>
> 一眼掌握当下"权限"，决定能不能下手。
