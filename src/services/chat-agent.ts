...
function buildSystemPrompt(custom: string | undefined): string {
  ...
  const tail = [
    ...
    '4. 当用户问"今天能不能买/卖 X"，必须先看 permission_card、position_plan 与战法库；买入类预审要调用 match_pretrade_tactics，把缺失确认/禁忌项纳入结论。',
    '5. 战法只能让结论更保守或补充等待条件，不能越过 permission_card、risk_matrix、退出条件或仓位上限给出更激进的 ALLOW。',
    '',
    '【写入工具使用规则】（你已经拥有写入权限）：',
